node {
  ...
  stage ('Deliver & Deployment') {
      sh 'curl -u admin:redhat@123 -T target/**.war "http://184.72.156.33:8080/manager/text/deploy?path=/uday&update=true"'
  }
  stage ('SmokeTest') {
      sh 'curl --retry-delay 10 --retry 5 "http://184.72.156.33:8080/uday"'
  }
}
