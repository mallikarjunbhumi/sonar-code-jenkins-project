node {
  ...
  stage ('Deliver & Deployment') {
      sh 'curl -u admin:redhat@123 -T target/**.war "http://54.234.164.115:8080/manager/text/deploy?path=/uday&update=true"'
  }
  stage ('SmokeTest') {
      sh 'curl --retry-delay 10 --retry 5 "http://54.234.164.115:8080/uday"'
  }
}
